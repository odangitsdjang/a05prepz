...
function includes (array, target) {
  ...
  return (array[0] === target) || includes(array.slice(1), target);
}
...
function numOccur (array, target) {
  if (array.length ===0) return 0;
  let rec = numOccur(array.slice(1), target);
  return ((array[0]== target) ? 1 : 0 ) + rec;
}
...
function sorted (array) {
  ...
  return (array[0] <= array[1]) && sorted(array.slice(1));
}
...
function factorialsRec (num) {
  ...
  prev.push(prev[prev.length-1] * num);
  return prev;
}
...
function range (min, max) {
  if (min > max) return [];
  let smaller = range(min, max-1);
  return smaller.concat([max]);
}
...
function exp (base, power) {
  ...
  if (power % 2 === 0 ) {
    // 3^ 8 === 3^ 4 * 3^4
    let y = exp(base, power/2);
    return y * y;
  } else {
    // 3^ 7 === 3^3 * 3^3 * 3
    let y = exp(base, Math.floor(power/2));
    return (y*y*base);
  }
}
...
function subsets (array) {
  ...
  let first = array[0];
  let others = subsets(array.slice(1));
  let subs = others.map((el)=>[first].concat(el));
  return others.concat(subs);
}
